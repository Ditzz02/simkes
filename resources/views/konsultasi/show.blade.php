                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        @if($konsultasi->qr_code)
                            <div>
                                <button class="btn btn-secondary bx bxs-printer" onclick="printQRCode()">Print QR Code</button>
                            </div>
                        ...
                        @if(Auth::guard('dosen')->check() && !$konsultasi->solusi)
                            <form action="{{ route('konsul-respond', $konsultasi->id) }}" method="POST">
                                @csrf
                                <div class="mb-3">
                                    <label for="solusi" class="form-label">Solusi</label>
                                    <textarea id="solusi" name="solusi" class="form-control" rows="4" required></textarea>
                                </div>
